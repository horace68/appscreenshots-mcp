...
让 AI Agent 在 [AppScreenshots](https://appscreenshots.net) 中创建可编辑的应用商店截图项目，使用授权素材、检查预览并导出 PNG/JPEG 或 ZIP。
...
> 使用我授权的截图素材，从可用模板创建一组英文 App Store 截图。标题简短、布局清晰。检查预览后导出图片，并给出可继续编辑的项目链接。

不支持文件传输的客户端，请先在网站上传素材，再授权项目或单独勾选素材。聊天附件不会自动变成 MCP 可读取的图片。如果客户端不能看图，应在网站检查预览。

初始云渲染额度为免费用户每日 30 帧、付费用户每日 300 帧，UTC 零点重置。预览和多语言图片均按帧计数，命中缓存不重复扣除。下载链接有效 15 分钟，产物保留 24 小时。套餐权限以网站和 `get_capabilities` 返回结果为准。
