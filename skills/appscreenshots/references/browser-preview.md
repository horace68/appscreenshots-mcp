...
- Read `get_project` immediately before `update_project`. Use its latest revision and a fresh idempotency key for a changed operation. `upsert_element` replaces the entire element; preserve existing fields. Reconcile conflicts instead of blindly repeating stale edits.
